...
ComputeProgram::ComputeProgram() : m_refCounter(0)
{
  program = NULL;
  compute = NULL;
}

ComputeProgram::ComputeProgram(const std::string& cs_path) : m_refCounter(0)
{
  ...
  if(!Compile(&tmpComputeSource))
  {
    std::cerr << "cs path: " << cs_path.c_str() << std::endl;
    throw std::runtime_error("compute shader compilation failed!");
  }

  if(!Link())
    throw std::runtime_error("compute shader linkage failed!");
...
ComputeProgram& ComputeProgram::operator=(const ComputeProgram& a_prog)
{
  this->program = a_prog.program;
  this->compute = a_prog.compute;
...
bool ComputeProgram::Compile(const char** computeSource)
{
  GLint compiled = 0;
  compute = glCreateShader(GL_COMPUTE_SHADER);

  glShaderSource(compute, 1, (const char**)computeSource, 0);
  ...
  if (!compiled)
  {
    ...
    glGetShaderInfoLog(compute, logLength, &charsWritten, log);
