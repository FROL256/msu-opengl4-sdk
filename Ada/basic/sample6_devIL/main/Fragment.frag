...
{	
  ...
  vec3 texColor = texture(colorTex, fragmentTexCoord).xyz;
  
  vec3 l = normalize(g_lightPos - fragmentWorldPos);
  vec3 diffuseTerm  = EvalDiffuseBRDFTerm(l, n, texColor*vec3(0.5,0.5,0.5)*fragmentFrontColor);
  vec3 specularTerm = EvalSpecularBRDFTerm_Phong(v, l, n, texColor*vec3(0.5,0.5,0.75)*fragmentFrontColor, 80);
