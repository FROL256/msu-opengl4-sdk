...
package body GL_HELPERS is
  ...
  function CreateGLTextureFromFile(a_fileName : String) return GLuint is

  begin
    return cpp_create_gl_texture_from_file(To_C(a_fileName));
  end CreateGLTextureFromFile;


  procedure DestroyGLTexture(a_texId : in out GLuint) is
    tmpId : aliased GLuint;
  begin
    tmpId := a_texId;
    glDeleteTextures(1, tmpId'Access);
    a_texId := -1;
  end DestroyGLTexture;

  procedure bindTexture(program : GLuint;  unit : GLint; name : String; texture : GLuint) is
    location : GLint;
  begin
    glActiveTexture(GLuint(integer(GL_TEXTURE0) + integer(unit)));     CHECK_GL_ERRORS;
    glBindTexture(GL_TEXTURE_2D, texture);   CHECK_GL_ERRORS;

    location := glGetUniformLocation(program, name);   CHECK_GL_ERRORS;
    if location >=0 then
  ...
  --pragma Linker_Options ("-lDevIL");
  --pragma Linker_Options ("-lILU");
  --pragma Linker_Options ("-lILUT");
  ...
  begin
    ...
    if self.vbo_tex > 0 then
      glDeleteBuffer(self.vbo_tex);
    end if;
  ...
  function CreateShapePlane(sizeX : float; sizeY : float; subdivsX : integer; subdivsY : integer) return GeometryShapeRef is
    ...
    texCroordLocation : GLuint := 1; -- simple layout, assume have tex at location = 1
    positions : array(0..subdivsX*subdivsY*24) of float;
    texCoords : array(0..subdivsX*subdivsY*12) of float;
  begin
    ...
    begin
      ...
      for x in 0..subdivsX-1 loop
        ...
        for y in 0..subdivsY-1 loop
          ...
          texCoords(texCoordPos + 0) := tx0;
          texCoords(texCoordPos + 1) := ty1;

          texCoords(texCoordPos + 2) := tx0;
          texCoords(texCoordPos + 3) := ty0;

          texCoords(texCoordPos + 4) := tx1;
          texCoords(texCoordPos + 5) := ty1;
          ...
          texCoords(texCoordPos + 6)  := tx1;
          texCoords(texCoordPos + 7)  := ty1;

          texCoords(texCoordPos + 8)  := tx0;
          texCoords(texCoordPos + 9)  := ty0;

          texCoords(texCoordPos + 10) := tx1;
          texCoords(texCoordPos + 11) := ty0;

          vertPos := vertPos + 4*6;
          texCoordPos := texCoordPos + 2*6;
    ...
    shape.vbo_tex := glGenBuffer;
    glBindBuffer(GL_ARRAY_BUFFER, shape.vbo_tex);                                  CHECK_GL_ERRORS;
    glBufferData(GL_ARRAY_BUFFER, GL_EXT_Float_Array(texCoords), GL_STATIC_DRAW);  CHECK_GL_ERRORS;
    ...
    glBindBuffer(GL_ARRAY_BUFFER, shape.vbo_tex);                                  CHECK_GL_ERRORS;
    glEnableVertexAttribArray(texCroordLocation);                                  CHECK_GL_ERRORS;
    glVertexAttribPointer(texCroordLocation, 2, GL_FLOAT, GLboolean(GL_FALSE), 0); CHECK_GL_ERRORS;
