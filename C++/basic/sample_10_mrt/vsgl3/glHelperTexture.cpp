...
RenderableTexture2D::RenderableTexture2D(GLenum format, GLsizei width, GLsizei height)
{
  ...
  m_coordTexture = TextureCreateEmpty(format, GL_RGB , width, height);
  m_texcoordTexture = TextureCreateEmpty(format, GL_RGB, width, height);
  ...
  glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, m_coordTexture, 0); CHECK_GL_ERRORS;
  glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, m_texcoordTexture, 0); CHECK_GL_ERRORS;
...
RenderableTexture2D::~RenderableTexture2D()
{
  ...
  glDeleteTextures(1, &m_coordTexture); m_coordTexture = -1;
  glDeleteTextures(1, &m_texcoordTexture); m_texcoordTexture = -1;
...
GLuint RenderableTexture2D::TextureCreateEmpty(GLint internalFormat, GLenum format, GLsizei width, GLsizei height)
{
  ...
  // Изменение !!! не будем использовать mip-map так иначе нам придеться генерировать их для текстуры глубины
  // А это в некоторых спецификациях запрещено :D
