...
ShaderProgram g_programDepthDraw;
...
GLUSboolean init(GLUSvoid)
{
  try 
  {
	...
	g_programDepthDraw = ShaderProgram("../main/DrawDepth.vert", "../main/DrawDepth.frag");
...
GLUSvoid reshape(GLUSuint width, GLUSuint height)
{
  ...
  glusPerspectivef(g_projectionMatrix.L(), 45.0f, (GLfloat) width / (GLfloat) height, 5.0f, 40.0f);  // Для большего контраста при рендере текстуры глубины плоскости отсечения почти зажимают объект
...
GLUSboolean update(GLUSfloat time)
{
  try 
  {
	...
	glUseProgram(g_programDepthDraw.program);
	glViewport(g_width / 2, 0 , g_width / 2, g_height / 2);
	bindTexture(g_programDepthDraw.program, 3, "inTex", pDebugTexture->GetDepthTexId());
	g_pFullScreenQuad->Draw();
	
	glUseProgram(g_programCopy.program);

	glViewport( 0 , g_height / 2 , g_width / 2, g_height / 2);
	bindTexture(g_programCopy.program, 3, "inTex", pDebugTexture->GetCoordTexId());
	g_pFullScreenQuad->Draw();

	glViewport(g_width / 2, g_height / 2, g_width / 2, g_height / 2);
	bindTexture(g_programCopy.program, 3, "inTex", pDebugTexture->GetTexCoordTexId());
	g_pFullScreenQuad->Draw();

	glViewport( 0 , 0 , g_width / 2, g_height / 2);
	bindTexture(g_programCopy.program, 3, "inTex", pDebugTexture->GetColorTexId());
	g_pFullScreenQuad->Draw();
