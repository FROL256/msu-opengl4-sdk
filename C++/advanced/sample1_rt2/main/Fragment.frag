...
float3 RayTrace(float x, float y, float w, float h)
{
  ...
  for(int i=0;i<1;i++)
  {
    ...
    float3 materialColorDiffuse  = g_matDiffuseColors[hit.materialId];
    float3 materialColorSpecular = g_matSpecularColors[hit.materialId];  
    ...
    float3 materialColorAmbient = g_matAmbientColors[hit.materialId] + g_matDiffuseColors[hit.materialId];
